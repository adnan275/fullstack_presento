...
1. Open the project root and navigate to the backend directory:
   ```bash
   cd backend
   ```
2. Install the backend dependencies:
   ...
   npm install
   ```
3. Create your `.env` configuration file by copying the example:
   ```bash
   cp .env.example .env
   ```
4. Update the backend `.env` variables with your MySQL, JWT, Cloudinary, Google OAuth, and Email configuration (see [Environment Variables Reference](#-environment-variables)).

### Step 2: Database Setup & Seed
1. Synchronize the Prisma schema and run migrations to create tables in your MySQL database:
   ```bash
   npx prisma migrate dev --name init
   ```
2. (Optional) Run the database seed script to populate products and initial data:
   ```bash
   npm run seed
   ```

### Step 3: Run the Backend API
Start the development server (runs on port `4000` by default):
```bash
npm run dev
```
...
3. Create your `.env` configuration file:
   ...
   touch .env
   ```
4. Define the API endpoint URL in `.env`:
   ```env
...
Presento comes with a built-in helper utility to access the admin product-management view:
1. Open the file [admin-helper.html](file:///Users/adnanrizvi/Desktop/fullstack_presento/admin-helper.html) in your browser.
2. Click **"Enable Admin Access"**. This sets `isAdmin = true` in your browser's local storage.
3. Click **"Go to Admin Page"** (or navigate to `/admin/products` on the frontend port) to access product addition and modifications page.
...
| :--- | :--- | :--- |
| `DATABASE_URL` | MySQL connection string | `mysql://root:password@localhost:3306/presento` |
| `JWT_SECRET` | Secret key used to sign JWT auth tokens | `your_super_secret_jwt_key` |
| `CLOUDINARY_CLOUD_NAME` | Cloudinary account name | `your_cloud_name` |
| `CLOUDINARY_API_KEY` | Cloudinary API Key | `your_api_key` |
| `CLOUDINARY_API_SECRET`| Cloudinary API Secret | `your_api_secret` |
| `GOOGLE_CLIENT_ID` | Google OAuth Client ID for Social login | `your_google_client_id` |
| `GOOGLE_CLIENT_SECRET` | Google OAuth Client Secret | `your_google_client_secret` |
| `EMAIL_HOST` | Host address of SMTP email server | `smtp.gmail.com` |
| `EMAIL_PORT` | Port of SMTP email server | `587` |
| `EMAIL_USER` | Email username used to send notifications | `your_email@gmail.com` |
| `EMAIL_PASSWORD` | App-specific password for SMTP user | `your_gmail_app_password` |
| `ADMIN_EMAIL` | Destination email for order logs | `presentotreasure@gmail.com` |
| `FRONTEND_URL` | Allowed origin for CORS validation | `http://localhost:5173` |

### Frontend (`frontend/.env`)

| Variable | Description | Value |
| :--- | :--- | :--- |
| `VITE_API_URL` | The REST API gateway address | `http://localhost:4000/api` |
